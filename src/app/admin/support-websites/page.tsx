...
import { Eye, Pencil, Send, Star } from "lucide-react";
...
import { getAllSupportSites, getAllSupportTickets, getSupportTemplates, saveSupportSite, saveSupportTemplate } from "@/lib/support-sites/store";
...
export default function AdminSupportSitesPage() {
  ...
  useEffect(() => {
    if (!isAdmin) return;
    Promise.all([getAllSupportSites(), getAllSupportTickets(), getSupportTemplates()]).then(([siteRows, ticketRows, templateRows]) => {
      setSites(siteRows); setTickets(ticketRows); setTemplates(templateRows);
    });
  }, [isAdmin]);
  ...
  async function toggleSite(site: SupportSite) {
    const next = { ...site, status: site.status === "published" ? "draft" as const : "published" as const };
  ...
  return <main className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
    ...
    <section className="mt-12"><h2 className="text-xl font-semibold">Customer websites</h2><p className="mt-1 text-sm text-muted-foreground">Review and publish a user&apos;s finished support website.</p><div className="mt-4 space-y-3">{sites.map((site)=><article key={site.id} className="flex flex-col gap-4 rounded-2xl border p-5 md:flex-row md:items-center"><div className="flex-1"><div className="flex items-center gap-2"><h3 className="font-semibold">{site.name}</h3><Badge variant={site.status==="published"?"default":"outline"}>{site.status}</Badge></div><p className="mt-1 text-sm text-muted-foreground">Owner: {site.userId} · /s/{site.slug}</p></div><div className="flex flex-wrap gap-2"><Button size="sm" variant="secondary" asChild><Link href={`/support-templates/${site.templateId}/editor?site=${site.id}`}>Edit site</Link></Button><Button size="sm" onClick={()=>toggleSite(site)}>{site.status==="published"?"Unpublish":"Publish website"}</Button>{site.status==="published"&&<Button size="sm" variant="ghost" asChild><Link href={`/s/${site.slug}`} target="_blank">Open</Link></Button>}</div></article>)}{!sites.length&&<div className="rounded-2xl border border-dashed py-12 text-center text-muted-foreground">No customer support websites yet.</div>}</div></section>
