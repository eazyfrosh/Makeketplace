...
import { getSupportSite, getSupportTemplate } from "@/lib/support-sites/store";
...
export default function TemplatePreviewPage() {
  ...
  useEffect(() => {
    ...
    getSupportTemplate(templateId).then((item) => setTemplate(item ?? undefined));
  }, [siteId, templateId]);
  ...
  return <div className="min-h-screen bg-muted/50">
    <header className="sticky top-0 z-50 flex h-16 items-center justify-between border-b bg-background px-4">
      <div><p className="font-semibold">{savedSite?.name ?? template?.name}</p><p className="text-xs text-muted-foreground">{savedSite ? "Saved website preview" : "Interactive template preview"}</p></div>
      ...
      <Button asChild><Link href={`/support-templates/${templateId}/editor${savedSite ? `?site=${encodeURIComponent(savedSite.id)}` : ""}`}>{savedSite ? "Edit Website" : "Customize Template"}</Link></Button>
    </header>
