...
export function SupportEditor({ templateId, siteId }: { templateId: string; siteId?: string }) {
  ...
  return <div className="flex min-h-screen flex-col bg-muted/40">
    ...
    <div className="grid flex-1 lg:grid-cols-[180px_minmax(0,1fr)_330px]">
      ...
      <aside className="border-l bg-background p-5"><h2 className="mb-5 font-semibold">{tab}</h2><div className="space-y-4"><Properties tab={tab} site={site} update={update} updateBranding={updateBranding} updateContact={updateContact}/></div></aside>
    </div>
  </div>;
}

function Properties({ tab, site, update, updateBranding, updateContact }: { tab: EditorTab; site: SupportSite; update: (patch: Partial<SupportSite>) => void; updateBranding: (key: keyof SupportSite["branding"], value: string | number) => void; updateContact: (key: keyof SupportSite["contact"], value: string) => void }) {
  if (tab === "Branding") return <><Field label="Business name" value={site.branding.businessName} onChange={(v) => updateBranding("businessName", v)}/><Field label="Logo URL" value={site.branding.logoUrl} onChange={(v) => updateBranding("logoUrl", v)}/><Field label="Primary color" type="color" value={site.branding.primary} onChange={(v) => updateBranding("primary", v)}/><Field label="Background" type="color" value={site.branding.background} onChange={(v) => updateBranding("background", v)}/><Field label="Text color" type="color" value={site.branding.text} onChange={(v) => updateBranding("text", v)}/><Field label="Font" value={site.branding.font} onChange={(v) => updateBranding("font", v)}/><Field label="Border radius" type="number" value={String(site.branding.radius)} onChange={(v) => updateBranding("radius", Number(v))}/></>;
