...
import { supportTemplates as builtInTemplates, createSupportSite as buildSupportSite } from "@/lib/support-sites/templates";
import { getPublishedTemplateSite, getSupportTemplates } from "@/lib/support-sites/store";
...
export function SupportTemplateMarketplace() {
  ...
  const [showcases, setShowcases] = React.useState<Record<string, SupportSite>>({});
  React.useEffect(() => {
    getSupportTemplates().then(async (items) => {
      setCatalog(items);
      const rows = await Promise.all(items.map(async (item) => [item.id, await getPublishedTemplateSite(item.id)] as const));
      setShowcases(Object.fromEntries(rows.filter((row): row is readonly [string, SupportSite] => Boolean(row[1]))));
    });
  }, []);
  ...
  return <div className="min-h-screen">
    ...
    <main className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8"><div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div className="relative w-full max-w-md"><Search className="absolute left-3.5 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search templates..." className="h-11 pl-10" /></div><div className="flex flex-wrap gap-2">{FILTERS.map(item => <button key={item} onClick={()=>setFilter(item)}><Badge variant={filter===item?"default":"outline"} className="cursor-pointer px-3 py-1.5">{item}</Badge></button>)}</div></div>
      <div className="mt-10 grid gap-7 md:grid-cols-2 xl:grid-cols-3">{templates.map(template => { const demo=showcases[template.id]??createSupportSite(template.id,"preview"); return <article key={template.id} className="glass group overflow-hidden rounded-3xl"><div className="h-56 overflow-hidden border-b border-border/60 bg-muted"><div className="pointer-events-none h-[520px] w-[180%] origin-top-left scale-[.46]"><SupportSitePreview site={demo} compact /></div></div><div className="p-6"><div><Badge variant="outline">{template.category}</Badge><h2 className="mt-3 text-xl font-semibold">{template.name}</h2></div><p className="mt-3 min-h-12 text-sm leading-6 text-muted-foreground">{template.description}</p><p className="mt-3 text-xs font-medium text-primary">Included with EazyTools All Access</p><div className="mt-6 flex gap-2"><Button variant="secondary" className="flex-1" asChild><Link href={`/support-templates/${template.id}/preview`}><Eye className="size-4" /> Preview</Link></Button><Button className="flex-1" asChild><Link href={`/support-templates/${template.id}/editor`}>Customize <ArrowRight className="size-4" /></Link></Button></div></div></article>})}</div>
